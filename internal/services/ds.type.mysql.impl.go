...
func (self *MySQLDataSourceTypeImpl) Sync(dataSourceId primitive.ObjectID) error {
...
func (self *MySQLDataSourceTypeImpl) CreateCatalog(catalogName string, dataSourceType models.DataSourceType, secret string) error {
	...
	if mysql.SSL {
		mysqlString = fmt.Sprintf("jdbc:mysql://%s:%s/%s?sslMode=REQUIRED", mysql.Host, strconv.Itoa(mysql.Port), mysql.Database)
	} else {
		mysqlString = fmt.Sprintf("jdbc:mysql://%s:%s/%s", mysql.Host, strconv.Itoa(mysql.Port), mysql.Database)
	}

	query := fmt.Sprintf("CREATE CATALOG data_source_%s USING mysql WITH (\"connection-url\" = '%s', \"connection-user\" = '%s', \"connection-password\" = '%s', \"case-insensitive-name-matching\" = 'true')", catalogName, mysqlString, mysql.User, mysql.Password)
